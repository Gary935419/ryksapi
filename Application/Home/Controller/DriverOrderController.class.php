...
class DriverOrderController extends CommonController
{
    ...
    public function lists()
    {
        ...
        switch ($data['taker_type_id']) {
            case 1: // 城际拼车
                $lists = $this->OrderIntercityModel->get_order_lists($con);
                break;
            case 2: // 市区出行
                $lists = $this->OrderTownModel->get_order_lists($con);
                break;
            case 3: // 同城货运
                $lists = $this->OrderTrafficModel->get_order_lists($con);
                break;
        }
    ...
    public function get_order_info()
    {
        ...
        $orderInfo = $this->OrderTrafficModel->getWhereInfo($orderInfoWhere);
