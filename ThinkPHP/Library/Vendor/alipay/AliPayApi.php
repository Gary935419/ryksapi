...
class AliPayApi{
     ...
     * @param float $money  交易金额 单位元
     * @param string $account   收款方账号
     * @param string $account_name  收款方姓名
     * @param string $notify_url    回调地址
     * @return string
     */
    public function payment($order_no, $money, $account, $account_name, $notify_url){
        $request = [
            ...
            "batch_fee"	=> $money,
            "batch_num"	=> 1,
            "detail_data"	=> "{$order_no}^{$account}^{$account_name}^{$money}^提现",
            "_input_charset"	=> "utf-8"
        ];
        ksort($request);
        $request_str = $this->createLinkstring($request);
